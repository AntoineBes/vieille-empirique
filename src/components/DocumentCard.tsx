...
export function DocumentCard({ document: doc, index }: DocumentCardProps) {
  ...
  return (
    <article className="card rounded-none border-x-0 border-t-0 py-5 px-0 group">
      <div className="flex items-start gap-4">
        {/* Numéro séquentiel */}
        {typeof index === "number" && (
          <span className="num-badge mt-1 shrink-0 w-6 text-right">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Résumé */}
          {doc.resume && (
            <p className="text-sm text-ink-500 mt-1.5 leading-relaxed line-clamp-2">
              {doc.resume}
            </p>
          )}
        </div>
